...
import piticket.py532lib.i2c as i2c
from piticket.py532lib.frame import Pn532Frame as Pn532Frame
...
class Mifare(i2c.Pn532_i2c):
    ...
    def mifare_change_keys(self,address,key_a,key_b):
        ...
        """
        if address < 128:
            # for the first 32 sectors of a 4KB card or 16 sectors of a 1KB card
            # divided into 4 blocks
            trailer_address = address | 3
        else:
            # For the 8 sectors of a 4KB card divided into 16 blocks
            trailer_address = address | 15
        data = self.mifare_read(trailer_address)
        data = key_a + data[6:10] + key_b
        self.mifare_write_standard(trailer_address,data)
